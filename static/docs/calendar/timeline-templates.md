---
sidebar_label: Playbooks & Key Dates
sidebar_position: 2
description: Apply Playbook templates to a DocJacket transaction. Set up key dates, tasks, and automations automatically by transaction type.
---
...
Playbook templates are pre-built workflows for different transaction types. Use one on a transaction and DocJacket can create the key dates, tasks, due-date rules, and automations that belong to that deal.

The **Key Date Library** holds reusable date rules. **Playbooks** consume those key dates and add the tasks, due-date offsets, phases, assignments, and automation that use them.
...
5. Choose a Playbook template that matches the transaction type, side, and state
6. Review the anchor dates, key dates, task counts, owner assignments, phases, and replacement option
7. Apply the template

This flow applies one Playbook template at a time. It is the best path when you want DocJacket to create or replace the transaction's key-date timeline and linked task workflow from the transaction's anchor dates.

When the template is applied, DocJacket creates the calculated key dates and any linked tasks for that transaction. If the transaction already has Playbook-generated items, the replacement option removes previously-applied template items while preserving manually added items and dates extracted from the contract.
...
For reusable date logic, manage the rule in **Templates > Key Date Library** so future Playbooks can use the same definition.
...
Go to **Templates → Playbooks** to create custom Playbook templates for your workflow. Use **Templates > Key Date Library** to manage reusable key date types and rules that Playbooks can consume.
