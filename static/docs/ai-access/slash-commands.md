...
:::note Not the same as Transaction Playbooks
These AI slash commands are a developer/power-user feature and are sometimes loosely called "playbooks." They are **not** [Transaction Playbooks](../templates/transaction-playbooks.md) — the reusable deal setups (formerly called checklists) under **Templates** that most people mean by "playbooks" in DocJacket.
:::
...
Each command chains DocJacket MCP tools — see the [full tool catalog](/docs/ai-access/tool-catalog) for what each one does.
...
The PDF-to-transaction workflow, which needs the actions tier. On a read-only or draft connection, use the in-app Upload Documents wizard for transaction creation. See the full reference at **[Contract Intake](/docs/ai-access/contract-intake)**.
