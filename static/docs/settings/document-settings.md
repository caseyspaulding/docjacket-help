...
Each type card shows the name, description, required/inactive badges, and how many documents currently use that type.

Document types are not matched on filenames. When a file goes through Upload & Extract or the packet splitter, DocJacket reads the document's content to detect its type; for a direct upload without extraction, the type is whatever you choose in the upload dialog or the checklist item you upload into.
...
`{DocType}` is the document's detected type — for example, a Colorado Amend/Extend form that goes through Upload & Extract is recognized from its content, so `{DocType}` resolves to "Amend & Extend" regardless of what the file was called. The filename is never used to decide the type. For a direct upload without extraction, `{DocType}` is the type you select in the upload dialog (or the checklist item you upload into); if none is selected, it falls back to "Document".
