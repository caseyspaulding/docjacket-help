...
These actions appear only on filed inbound emails, and they are the fix when a message lands on the wrong transaction. Nothing is lost — the message and its attachments travel with it. See [Getting Email Into DocJacket](./getting-email-into-docjacket.mdx) for how messages get filed in the first place.
