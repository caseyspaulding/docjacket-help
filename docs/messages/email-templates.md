...
## Refresh transaction data in a draft

Save the transaction's date changes first. In the template-based composer, click **Refresh**. The tooltip says “Refresh the transaction data in this email draft.” This rerenders the template's subject and message with the latest saved transaction values, replacing any edits to that text. Your recipients, attachments, and selected signature stay as they are. An open draft does not update automatically, and refresh never sends the email.
