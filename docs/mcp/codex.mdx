  ...
  "mainEntity": [
    {
      ...
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "In Codex (or ChatGPT with MCP connectors enabled), add a new MCP server with the URL https://mcp.docjacket.com/mcp. Complete the OAuth consent screen DocJacket shows. Codex will load 10 read tools and you can ask questions about your DocJacket pipeline."
      }
    },
    {
      "@type": "Question",
...
6. Run `codex tools list` — you should see 10 DocJacket tools.

For the full plugin (with the Daily Triage skill and `.codex-plugin` package), see [DocJacket-LLC/codex-plugin](https://github.com/DocJacket-LLC/codex-plugin) on GitHub.

<ScreenshotPlaceholder>Codex CLI after connecting — 10 DocJacket tools listed</ScreenshotPlaceholder>
...
6. Start a new chat — DocJacket's 10 tools are now available.
...
Codex / ChatGPT will use these as appropriate based on your question. You don't need to remember the names.

| Tool | Use it for |
|---|---|
| `search_transactions` | "Search DocJacket for…" |
| `get_transaction` | "Tell me about the [property] deal" |
| `find_transaction_by_property` | "Find the deal at 1234 Main" (fuzzy address) |
| `get_open_tasks` | "What's on my task list?" |
| `get_key_dates` | "When's inspection on [property]?" |
...
The Codex plugin ships a **Daily Triage** skill that wraps `get_next_required_actions` with formatting and rationale-rendering.
...
Either way you get a ranked list grouped by urgency, with what-to-do-next bullets per transaction.
