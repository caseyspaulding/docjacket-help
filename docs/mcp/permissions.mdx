  ...
  "mainEntity": [
    {
      ...
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Three: read, draft, and actions. Read covers every lookup tool (search transactions, get key dates, list tasks, etc.). Draft and actions both cover writes — draft for tools that prepare emails or task suggestions, actions for sending and modifying. Every write tool prompts the assistant to ask for your confirmation in chat before firing — the conversation is the approval gate."
      }
    },
    {
      "@type": "Question",
