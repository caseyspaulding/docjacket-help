...
sidebar_position: 6
description: Every operation in the DocJacket REST API — 120 endpoints grouped by resource, with method, path, and required scope. Generated from the OpenAPI specification.
---
...
The DocJacket REST API lets your own systems read and act on transactions, contacts, tasks, key dates, documents, extractions, disclosures, and templates. This page lists **all 120 operations** in one place, grouped by resource, so the whole surface is readable without running JavaScript or parsing the raw specification.
...
120 operations across 14 sections.
...
| POST | `/api/v1/transactions/{transactionId}/emails/agent` | Queue an email to an agent on the deal for approval | `actions` |
| POST | `/api/v1/transactions/{transactionId}/emails/agent-followup` | Queue a follow-up email to an agent or vendor for approval | `actions` |
| POST | `/api/v1/transactions/{transactionId}/emails/client-update` | Queue a status-update email to a deal party for approval | `actions` |
| POST | `/api/v1/transactions/{transactionId}/emails/document-request` | Queue a document-request email to a deal party for approval | `actions` |
...
| Method | Path | Description | Scope |
|---|---|---|---|
| POST | `/api/v1/account/deletion/cancel` | Cancel a scheduled workspace deletion | `actions` |
| POST | `/api/v1/account/deletion/request` | Schedule this workspace for deletion after the grace period | `actions` |
| GET | `/api/v1/account/deletion/status` | Whether workspace deletion is scheduled, and whether the caller may act on it | `read` |
| GET | `/api/v1/account/export/latest` | Status of the most recent workspace data export | `read` |
| POST | `/api/v1/account/export/request` | Start building a full export of this workspace's data | `actions` |
