...
Choose **Refresh from transaction**, review the proposed subject and message, then choose **Replace subject and message** or **Keep current draft**. Replacement keeps recipients, attachments, and the selected signature. Refreshing does not send the email. Save the transaction date first.
