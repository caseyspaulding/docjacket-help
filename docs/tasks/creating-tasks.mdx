...
DocJacket adds the checklist's tasks and keeps them grouped under the checklist name. If the checklist includes due-date rules, those dates are calculated from the transaction's key dates. You can also create or replace key-date-and-task workflows from the transaction overview with **Generate Timeline**.
