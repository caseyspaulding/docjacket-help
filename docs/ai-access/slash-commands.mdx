...
**Chains:** `request_upload_url` → `kick_off_extraction` (or `upload_document_for_extraction` for small PDFs) → `get_extraction_results` (polling) → review → `apply_extraction` → `add_key_dates_batch` → `render_email_template` + `send_*` intros → `create_reminder` for major deadlines → `get_intake_status` final summary.
