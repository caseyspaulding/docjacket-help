...
1. **Upload.** The assistant uses a two-step presigned upload by default: `request_upload_url` returns a short-lived URL the assistant's client PUTs the PDF bytes to directly, then `kick_off_extraction` starts the pipeline against the uploaded blob. PDF bytes never enter the chat conversation, so a 30-page contract doesn't burn your conversation's context window. For small PDFs or clients that can't follow a presigned URL, there's also a single-step `upload_document_for_extraction` fallback. Either path returns an `extractionJobId` and kicks off extraction in the background.
