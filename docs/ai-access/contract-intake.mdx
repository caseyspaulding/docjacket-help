  ...
  "mainEntity": [
    {
      ...
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "External AI contract intake is available only when your AI Access connection has the required action tools enabled. In the standard read-first setup, use DocJacket's in-app Upload & Extract workflow for contract creation. When action tools are enabled, the assistant can run extraction, show extracted parties, addresses, key dates, and financials for review, and apply the result only after you confirm in chat."
      }
    },
    {
      "@type": "Question",
