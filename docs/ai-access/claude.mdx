  ...
  "mainEntity": [
    {
      ...
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "In Claude.ai, open Settings → Connectors → Add custom connector. Paste https://mcp.docjacket.com/mcp as the server URL, click Continue, and complete the consent screen. Claude loads the DocJacket tools available to your granted scopes. The current customer-facing surface is read-first, so you can start asking questions about your DocJacket pipeline right away."
      }
    },
    {
      "@type": "Question",
