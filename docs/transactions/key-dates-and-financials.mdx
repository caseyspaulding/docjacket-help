...
Every transaction has dates and dollar amounts that need to be tracked carefully. DocJacket keeps the timeline and financial summary organized on the transaction overview.

## Key dates

The key-date timeline on the overview shows your most important deadlines:
...
Some dates also appear as canonical key dates on the calendar so tasks, reminders, and reporting can use the same anchor date.

### Editing dates

Use the transaction edit page or the key-date timeline controls to adjust dates. If a date came from extraction, review it before saving so downstream tasks and reminders use the correct anchor.
