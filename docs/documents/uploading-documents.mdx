---
sidebar_label: Uploading Documents
sidebar_position: 2
description: Upload documents to a DocJacket transaction — drag-and-drop, email forwarding, Google Drive import, and bulk upload.
---

import ScreenshotPlaceholder from '@site/src/components/ScreenshotPlaceholder';

<head>
<script type="application/ld+json">
{JSON.stringify({
  "@context": "https://schema.org",
  "@type": "HowTo",
  "name": "How to upload documents to a DocJacket transaction",
  "description": "Drag and drop one or more files into a DocJacket transaction's Documents tab — PDFs, images, and Office files are accepted, and DocJacket auto-categorizes them on upload.",
  "totalTime": "PT1M",
  "step": [
    {
      "@type": "HowToStep",
      "name": "Open the Documents tab",
      "text": "Open the transaction you want to add files to and click the Documents tab.",
      "url": "https://help.docjacket.com/docs/documents/uploading-documents#drag-and-drop"
    },
    {
      "@type": "HowToStep",
      "name": "Select files to upload",
      "text": "Click the Upload button to browse for files, or simply drag the files onto the page. Multiple files can be uploaded at once.",
      "url": "https://help.docjacket.com/docs/documents/uploading-documents#drag-and-drop"
    },
    {
      "@type": "HowToStep",
      "name": "Drop the files",
      "text": "Release the files in the upload zone. DocJacket accepts PDFs (most common for contracts and disclosures), images (JPG, PNG, TIFF), and Office files (Word, Excel).",
      "url": "https://help.docjacket.com/docs/documents/uploading-documents#supported-file-types"
    },
    {
      "@type": "HowToStep",
      "name": "Review auto-categorization",
      "text": "DocJacket reads the file name and content to assign a category — Purchase Agreement, Inspection Report, Title Commitment, and so on. Change the category manually if the AI guesses wrong.",
      "url": "https://help.docjacket.com/docs/documents/uploading-documents#automatic-categorization"
    }
  ]
})}
</script>
</head>

# Uploading Documents

Getting documents into DocJacket is quick and easy. Here's how.

## Drag and drop

The fastest way. Open a transaction's **Documents** tab and:

1. Click the **Upload** button (or just drag files onto the page)
2. Drop one or more files
3. DocJacket uploads them and automatically assigns a category based on the file name and content

<ScreenshotPlaceholder>upload area with drag and drop zone</ScreenshotPlaceholder>

## Supported file types

DocJacket accepts most common file types:

- **PDF** — The most common format for contracts and disclosures
- **Images** — JPG, PNG, TIFF (great for photos of signed documents)
- **Office files** — Word docs, Excel spreadsheets
- **Other** — Most standard document formats

## Automatic categorization

When you upload a document, DocJacket looks at the file name and content to guess the right category — things like "Purchase Agreement", "Inspection Report", or "Title Commitment". You can always change the category manually if the AI gets it wrong.

## Uploading multiple files

You can upload multiple files at once. Just select them all or drag a batch into the upload area. Each file is processed individually and categorized on its own.

## Other ways documents get into DocJacket

Documents don't only come from manual uploads:

- **AI Extraction** — When you create a transaction via Upload & Extract, the contract is automatically saved to the Documents tab
- **DocDrop** — Contacts can email documents directly into the transaction (see [DocDrop](./docdrop.mdx))
- **Google Drive** — Import files from your Drive (see [Google Drive Integration](./google-drive-integration.mdx))
