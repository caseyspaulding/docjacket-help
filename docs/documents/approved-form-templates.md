...
They live inside an office, not the main Templates page. Open **Offices**, choose an office, then use the **Approved forms** tab.
...
After upload, DocJacket opens the template editor.
...
Click a field to edit its role, label, binding, or placement. Use zoom and page controls when working with long forms.

## Save and publish

Click **Save Draft** while mapping fields. Drafts do not send.

Click **Publish** when the template is ready. Published versions are locked so Signature Requests already in flight continue to render against a stable form layout.
...
Archiving a template does not break in-flight Signature Requests.
