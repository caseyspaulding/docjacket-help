...
3. Find the **Forms & signatures** panel
...
In the prepare step, choose the templates for the packet and review the autofill preview.
...
If required office data is missing, sending can be blocked until the office setup is corrected.
...
After sending, the Signature Request is immutable. Sent packets cannot be edited or added to because the audit trail and signed PDF must remain stable.
...
In-flight packets refresh automatically. You can also refresh manually or cancel a packet while cancellation is still allowed.
